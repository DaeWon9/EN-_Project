...
namespace Library.Controller
{
    class Administrator : MenuSelection // 기능별로 쪼개기.. 모듈화...
    {
            ...
            while (!isInputEscape && !isSearchBookCompleted)
            {
                ...
                switch (currentConsoleCursorPosY)
                {
                    ...
                    case (int)Constant.BookSearchPosY.ISBN:
                        bookISBN = DataProcessing.GetDataProcessing().GetInputValues(administratorScreen, Constant.SEARCH_POS_X, (int)Constant.BookSearchPosY.ISBN, Constant.MAX_LENGTH_BOOK_ISBN, Constant.TEXT_PLEASE_INPUT_NUMBER, Constant.EXCEPTION_TYPE_NUMBER_SPACE_PYPHEN, Constant.EXCEPTION_TYPE_ANY);
                        break;
                    ...
                    case (int)Constant.BookSearchPosY.SEARCH:
                        isSearchBookCompleted = IsSearchBookCompleted(administratorScreen, bookId, bookName, bookPublisher, bookAuthor, bookISBN, bookPrice, bookQuantity);
                        break;
                    default:
                        break;
                }
        ...
        private bool IsSearchBookCompleted(AdministratorScreen administratorScreen, string bookId, string bookName, string bookPublisher, string bookAuthor, string bookISBN, string bookPrice, string bookQuantity)
        {
            ...
            if (getYesOrNoBySearching == Constant.INPUT_ENTER && isSearchAndModify == Constant.IS_SEARCH_AND_BORROW) // 검색 후 수정까지하는 함수 -> 여기서 검색된 도서 id 리스트 만들어서 대여할때 중복체크하기
            {
                ...
                administratorScreen.PrintSelectedValues(DataBase.GetDataBase().Select(Constant.FILED_ALL, Constant.TABLE_NAME_BOOK, DataProcessing.GetDataProcessing().GetConditionalStringBySearchBook(bookId, bookName, bookPublisher, bookAuthor, bookISBN, bookPrice, bookQuantity)), Constant.TABLE_NAME_BOOK, Constant.TEXT_NONE);
            ...
            while (!isLogout)
            {
                ...
                switch (menuValue)
                {
                    ...
                    case (int)Constant.AdministratorMenu.SEARCH_BY_NAVER:
                        naverBook.SearchBookByNaver(administratorScreen);
                        break;
